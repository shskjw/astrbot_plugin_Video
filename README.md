...
插件统一使用一个模型配置项，并支持 **视频预设**、**黑白名单**、**次数限制**、**签到** 与 **统计**，通过 `_conf_schema.json` 让用户自行配置：
...
2. 解析视频预设并组合最终提示词
3. 按消息链顺序提取图片
4. 如果存在回复图片，则按回复链顺序继续提取
5. 如果回复链为空，会尝试主动读取被引用消息
6. 兼容识别图片组件、文本图片链接、data URL、base64:// 来源
7. 创建异步任务
8. 立即发送“任务已提交”
9. 后台继续处理
10. 生成完成后主动通知用户
...
- `main.py`：AstrBot 插件入口、LLM Tool、预设管理、次数管理、签到与统计命令
- `models.py`：数据模型
- `exceptions.py`：异常定义
